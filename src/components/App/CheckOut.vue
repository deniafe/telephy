<template>
  <v-card flat color="accent" tile>
    <v-card height="5px" tile text color="appGreen"> </v-card>
    <div class="text-center pt-6">
      <v-icon size="70">mdi-{{ icon }}</v-icon>
    </div>
    <div class="text-center pt-6 text-h6 font-weight-bold">{{ name }}</div>
    <div class="text-center pt-6 text-h4 font-weight-bold appBlue--text">
      {{ amount }}<span class="text-subtitle-1 appWhite--text">/month</span>
    </div>
    <v-divider class="mx-4 my-8"></v-divider>
    <v-list dense color="accent" flat>
      <v-list-item-group color="accent">
        <v-list-item v-for="(item, i) in items" :key="i">
          <v-list-item-icon>
            <v-icon color="appGreen">mdi-checkbox-marked</v-icon>
          </v-list-item-icon>
          <v-list-item-content>
    ...
    <v-card-actions v-if="!currentPlan" class="py-4 px-8">
      ...
      >
        <!-- <i class="fas fa-money-bill-alt"></i>
          Make Payment -->
        <v-btn text outlined color="appGreen">Pay in NGN</v-btn>
      </paystack>
      <v-spacer></v-spacer>
      <v-btn text outlined color="appYellow" @click="makePayment"
        >Pay in USD</v-btn
      >
    </v-card-actions>
