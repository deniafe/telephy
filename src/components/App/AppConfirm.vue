      ...
      <v-card-text>
        <v-container>
          <div class="d-flex justify-center">
            <v-icon color="appPink" x-large>mdi-help-circle-outline</v-icon>
          </div>
        </v-container>
      </v-card-text>
      <v-card-actions>
        <v-btn
          :color="confirmationYes !== 'Yes Upgrade' ? 'appBlue' : 'appPink'"
          class="ml-4"
          text
          @click="closeDialog"
        >
        ...
        <v-spacer></v-spacer>
        <v-btn
          :color="confirmationYes !== 'Yes Upgrade' ? 'appPink' : 'appBlue'"
          class="mr-4"
          text
          @click="deleteAction"
        >
